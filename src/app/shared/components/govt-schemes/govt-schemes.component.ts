...
@Component({
  ...
  styleUrls: ['./govt-schemes.component.scss']
})
export class GovtSchemesComponent implements OnInit, OnDestroy {
   ...
   ngOnInit(): void {
    ...
    this.subscription = this.apiService.currentServiceType.subscribe(
      currentservice => {
        ...
        const data: any = {}
        data.schemeName = this.rtiDetailsForm.get('schemeName').value;
    ...
    this.subscriptionTwo = this.apiService.subscribeRtiData.subscribe(
      rtiData => {
        ...
        if (this.selectedRoute === rtiData) {
          this.apiService.postGovtSchemesService(data)
            .subscribe(
              (resultID: any) => {
